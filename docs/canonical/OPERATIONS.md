...
**Last verified against live tree:** 2026-06-09 (Cowork ground-truth pass)
...
| **Information Booth** (`/booth`) and **Where the Money Goes** (`/money`) | `src/routes/InfoBooth.jsx` + `InfoBooth.css`; `src/routes/Money.jsx` + `Money.css`. **`/money` imports `InfoBooth.css` and reuses its class names** — booth as literal template (F3 2026-08-03), so `InfoBooth.css` is furniture for two rooms while named for one. Edits to the booth's sheet, credo, rule, question or contact rules land on both. **C2 2026-08-03:** the room shipped for one unpushed commit as "The Foundation" at `/foundation` (`Foundation.jsx`/`.css`, `.fnd-` prefix, `data-room="foundation"`) — Mike ruled the word carries a legal expectation the charter refuses. All of it is renamed and **no redirect exists**; if you find `/foundation` referenced anywhere outside a historical comment, it is a miss. |
