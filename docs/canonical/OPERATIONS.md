...
| **Information Booth** (`/booth`) and **The Foundation** (`/foundation`) | `src/routes/InfoBooth.jsx` + `InfoBooth.css`; `src/routes/Foundation.jsx` + `Foundation.css`. **The Foundation imports `InfoBooth.css` and reuses its class names** — booth as literal template (F3 2026-08-03), so `InfoBooth.css` is furniture for two rooms while named for one. Edits to the booth's sheet, credo, rule, question or contact rules land on both. |
