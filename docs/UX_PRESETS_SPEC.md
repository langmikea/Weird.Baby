...
5. **Shared-preset landing** (`UX_SPEC §C.5.0` Q3) — does `weird.baby/p/<id>` land at the Lobby first (default per §L.1) or directly in the preset state? Canon default is Lobby-first; confirm. **[CONFIRMED 2026-06-07, Mike: Lobby-first. Built: `/p/<id>` resolves the preset via `GET /api/presets/<id>`, parks the snapshot in sessionStorage, lands at the Lobby; the exhibit applies it once on arrival with Play semantics (§3). Unknown ids degrade to a plain Lobby visit.]**
...
**Factory recipe repair (2026-06-07, found during live verification of phase 1):** four of the five v28-ported factory recipes filtered on namespaces the hunter_root export does not carry (`src`/`era`/`type` with values `press`/`stage`/`seeds`/`video`); `matchFilter` reads only discovered `HR_DIMENSIONS` (album/people/bands/source/content_kind/card_kind), so they committed an empty filter — silent no-ops on desktop and mobile (gold applied-highlight, wall unchanged). Per Mike ("remove or repair"): **repaired** "Years past" → `album: medusas_disco` (the recipe's `era: medusas` intent, 3 artifacts); **removed** "Press clippings" / "Live captures" / "Video evidence" — no filterable equivalent exists (press/stage/video content is tagged only in `unsorted`, which `buildDimensions` excludes). Factory set is now: Surprise me, Years past. Re-add by recipe only when the export carries the tags.
...
Still open after this build: §8.1 mobile presets phase 2 (factory Show + mobile peek-return chip; phase 1 apply-only pills live 2026-06-07). Idle auto-return BUILT 2026-06-07 (song change + idle, see §3/§5 #3). **Sharing BUILT 2026-06-07:** `weird.baby/p/<shortid>` live — Share verb per filled slot publishes the snapshot (same wire shape as localStorage persistence) to D1 via `POST /api/presets`, copies the link; Lobby-first landing per §5 #5. What remains of §0 is the fuller **preset-as-artifact model** — operator tagging/featuring/retiring of presets and entry-state-as-preset (`UX_LIFECYCLE_SPEC §4.2`); the share URL is the visitor-facing slice, not the whole model. O9 shuffle/loop WIRED 2026-06-07 (524bf41): player-owned state crossed at this same seam; shuffle randomizes the next-up queue, loop replays the current selection on end (controls §9.2), verified live.
