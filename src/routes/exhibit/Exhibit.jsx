...
import InstrumentPanel from "./instrument-panel.jsx";
  ...
  return (
    <ol className="tl-tracks">
      {album.tracks.map((track, ti) => {
        ...
        /* ═══ [2026-08-26] A ROW MAY *RUN* RATHER THAN OPEN ═══════════════════
           MIKE: **"Clicking TERMINAL.EXE should not pop up an intermediary page
           that has a run button; it should run it instead."**
           IT IS THE THIRD BRANCH ON A CLICK, NOT A NEW MECHANISM. `jumpTo`
           already established that a row can do something other than open its
           face, and for the same reason: some rows are not pages. This one
           dispatches a declared event with a declared detail and stops — the
           engine learns nothing, exactly as it learns nothing from `jumpTo`
           being an album id.
           A RUNNING ROW HAS NO FACE AT ALL, which is what removes the
           intermediary page rather than hiding it: there is nothing behind the
           row to land on, so closing what it opened returns to the tracklist. */
        const runs = !!track.run && !!track.run.event;
        const selectable = hasVids || !!track.face || jumps || runs;
            ...
            onClick={() => { if (!selectable || skipped) return;
                             if (runs) {
                               window.dispatchEvent(new CustomEvent(
                                 track.run.event, { detail: track.run.detail || {} }));
                               return;
                             }
...
/* ======== [P2 2026-08-02] THE INSTRUMENT PANEL — MOVED 2026-08-26 =======
   `InstrumentPanel` and its six helpers now live in `./instrument-panel.jsx`,
   imported at the head of this file. **TERMINAL.EXE is why:** the feed panel's four
   bays are drawn on a Portal screen by `RobotsExhibitFlow.jsx` as well as on
   the album page here, and a second caller in another file needed the
   component exported — which on a file that default-exports a component costs
   `react-refresh/only-export-components`. `use-yt-player.js` was extracted for
   the identical trade on 2026-08-21 and this follows it.
   Nothing about the panel changed in the move. Its reasoning travelled with
   it, whole, and that file is where it is now read. ======================= */
                          ...
                          <button
                            ...
                            onClick={() => window.dispatchEvent(
                              new CustomEvent(face.action.event, { detail: {
                                ...
                                /* [2026-08-26] TERMINAL.EXE RIDES THE SAME DETAIL.
                                   `kind` is what the overlay branches on and
                                   `boot`/`panel` are what a console needs; all
                                   three are forwarded exactly as `src` and
                                   `frameTitle` already were. THE ENGINE STILL
                                   LEARNS NOTHING — it copies declared fields
                                   onto an event and stops. An action that
                                   declares none of them dispatches what it
                                   dispatched before, key for key, because
                                   `undefined` is what a field it never had
                                   already resolved to. */
                                kind: face.action.kind,
                                boot: face.action.boot,
                                panel: face.action.panel,
                                /* THE BEZEL IS THE SET, NOT THE PICTURE, and it
                                   has to ride too: `PortalScreen` falls back to
                                   a bare picture when it is handed none, which
                                   on the console measured as a console with no frame
                                   and — worse — NO WAY OUT, because the [X]
                                   lives on the strip the frame carries. Caught
