...
export default function App(){
  ...
  const [mikeModeUnlocked,setMikeModeUnlocked]=useState(false);
  const mikeKeyLog=useRef([]);
  useEffect(()=>{
    const handler=(e)=>{
      if(e.key.toLowerCase()!=="m") return;
      const now=Date.now();
      mikeKeyLog.current=[...mikeKeyLog.current.filter(t=>now-t<1000),now];
      if(mikeKeyLog.current.length>=3){ setMikeModeUnlocked(u=>!u); mikeKeyLog.current=[]; }
    };
  ...
  return(
    <div style={{minHeight:"100vh",background:theme.bg,color:theme.textPrimary,fontFamily:"Georgia,serif",padding:"18px 14px",transition:"background 0.4s"}}>
      <div style={{maxWidth:740,margin:"0 auto"}}>
        ...
        {/* ── ARTIST SELECTOR ── */}
          <div style={{display:mikeModeUnlocked?"flex":"none",justifyContent:"center",gap:0,marginBottom:18,flexWrap:"wrap"}}>
