...
import { FACTS } from "./hr_facts.js";
import HrExhibitFlow from "./HrExhibitFlow.jsx";
...
function buildPlayQueue(album, startTi, selVisMap) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (sel && sel.size === 0) continue; // user-skipped
...
// ─── YOUTUBE PLAYER HOOK ──────────────────────────────────────────────────────
// One player, initialized lazily on first loadVideo call.
// containerRef must point to a stable DOM node that does not move.
...
// ─── SPLIT PERSISTENCE ────────────────────────────────────────────────────────
const SPLIT_KEY = "wb-hr-split";
const SPLIT_MIN = 25; const SPLIT_MAX = 75;
const CF_KEY    = "wb-hr-cfh";
    ...
    <div className="cf-wrap" style={{ height: cfH }}
      ...
      {SPINE.map((a,i) => {
        ...
        return (
          ...
          >
            <img src={a.art} alt={a.title} loading="lazy" />
...
// ─── TRACKLIST ────────────────────────────────────────────────────────────────
// Pure left-column component. No video panel inside.
...
function PlayerBar({ video, track, album, onSkipBack, onSkipForward, canSkipBack, canSkipForward, onTogglePlay, onToggleMute, onSetVolume, getState }) {
  ...
  // Poll state for play/mute indicators
  useEffect(() => {
    if (!video) return;
  ...
  return (
    <div className="pb">
      ...
      <div className="pb-controls">
        ...
        {/* CC / subtitles */}
        <button className="pb-ctrl" onClick={() => {
          // Toggle YT captions module if available
          const p = window.document.querySelector('iframe')?.contentWindow;
          // Fallback: just toggle a visual indicator
...
export default function HrSpine() {
  ...
  const [active, setActive]             = useState(4);       // carousel focus
  const [activeDisplay, setActiveDisplay] = useState(4);     // open tracklist
  const debounceRef = useRef(null);

  // Per-album state (lifted so album switches don't reset selections)
  ...
  // YT player — lives in a stable div in the right column, never moves
  ...
  useEffect(() => {
    ...
    fetch("/api/visits", { method:"POST", headers:{"Content-Type":"application/json"},
      body: JSON.stringify({ page:"/hr", referrer:document.referrer }) }).catch(()=>{});
  }, []);
  ...
  // What to show in the right column
  ...
  // Thumbnail for right panel when nothing playing
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        /* VIDEO PANEL — stable right column */
        .vp-area{position:relative;width:100%;background:#0a0a0a;border:1px solid #1e1e1e;flex-shrink:0}
        .vp-area::before{content:"";display:block;padding-top:56.25%} /* 16:9 */
        .vp-inner{position:absolute;inset:0}
        /* YT player div fills this completely */
        .yt-player{width:100%;height:100%;display:block}
        /* Thumbnail overlay — sits above yt-player when nothing playing */
        ...
        /* AUDIO-ONLY OVERLAY — focused album art, centered, classy */
      ...
      <div className={`hr-root${visible?" visible":""}`}>
        ...
        <div className="hr-nav">
          <button className="hr-nav-logo" onClick={() => navigate("/shop?from=hr")}>Weird.Baby</button>
          <div className="hr-nav-sub">Hunter Root</div>
          <button className="hr-nav-return" onClick={() => navigate("/shop?from=hr")}>Gift Shop</button>
        </div>
        ...
        <div className="hr-main hr-snap">
          <div className="hr-main-inner" ref={bodyRef}
            ...
            <div className="hr-right">
              {/* VIDEO AREA — YT player div lives here, never moves */}
              <div className="vp-area">
                <div className="vp-inner">
                  {/* The actual YT player — permanent, stable DOM node */}
        ...
        {/* EXHIBIT FLOW — P2 (Archive) + P3 (Artifacts) + P4 (Exit) */}
