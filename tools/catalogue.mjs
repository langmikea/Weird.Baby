...
/* the Sunday questions file, rendered under the catalogue so the sitting has one page */
const QFILE = path.join(HERE, "..", "docs/desk/QUESTIONS-for-Sunday.md");
const questions = (() => {
  let md; try { md = fs.readFileSync(QFILE, "utf8"); } catch { return ""; }
  const items = md.split(/\n(?=\d+\. )/).slice(1).map(b => b.replace(/^\d+\. /, "").replace(/\n\s+/g, " ").trim());
  if (!items.length) return "";
  const inline = t => esc(t).replace(/\*\*(.+?)\*\*/g, "<b>$1</b>").replace(/(https?:\/\/[^\s)]+)/g, '<a href="$1">$1</a>');
  return `<h2>Questions for this sitting<small>${items.length}, from docs/desk/QUESTIONS-for-Sunday.md</small></h2>
<ol class="qs">${items.map(t => `<li>${inline(t)}</li>`).join("")}</ol>`;
})();
...
.qs{max-width:78ch;padding-left:1.4em}.qs li{margin:10px 0}.qs a{color:var(--gold-ink)}
