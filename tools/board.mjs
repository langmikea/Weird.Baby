...
/* THE DROPS: what the world sees each day, for the calendar. [Mike, 2026-09-18: "it should indicate the
   feature drop for each day ... the number drop and any other drop"] Feature drops from the catalogue's
   day, the Number from its ledger, the daily Q&A from its first day, the ask reel on day 1 and each
   character's shoot reel on its arrival day (RUN-OF-SHOW-20260918.md). docs/desk/DROPS.json; the
   workbook's Calendar reads it. */
const drops = {}, drop = (date, kind, label) => (drops[date] ||= []).push({ kind, label });
if (CAT.run?.start) {
  const byDay = {};
  for (const r of CAT.rows) if (typeof r.day === "number" && String(r.ruling || "").toLowerCase() !== "c") (byDay[r.day] ||= []).push(r);
  for (const [n, rows] of Object.entries(byDay)) {
    const date = addDays(CAT.run.start, Number(n) - 1), who = rows.find(r => r.kind === "character");
    const lead = who || rows.find(r => r.kind === "feature" || r.kind === "game" || r.kind === "setting" || r.kind === "engine") || rows[0];
    drop(date, "feature", `Day ${n}: ${lead.name}${rows.length > 1 ? ` +${rows.length - 1}` : ""}`);
    /* the other reels first go out the Monday after the door [Mike, 2026-09-18] */
    const firstMon = addDays(CAT.run.start, (8 - dow(CAT.run.start)) % 7 || 7);
    if (Number(n) === 1) drop(firstMon, "reel", "Ask reel, noon");
    if (who) drop(date < firstMon ? addDays(firstMon, 1) : date, "reel", `Shoot reel: ${who.name}`);
  }
  const last = addDays(CAT.run.start, Math.max(...Object.keys(byDay).map(Number)) - 1);
  for (let d = CAT.run.start; d <= last; d = addDays(d, 1)) drop(d, "qa", "Answer of the day");   // from the door, daily, forever
}
try { for (const r of read("reels/numbers.json").rows) if (r.status !== "held" && r.piece) drop(r.date, "number", `Number: ${r.piece}`); } catch {}
fs.writeFileSync(path.join(REPO, "docs/desk/DROPS.json"), JSON.stringify({ _: "What drops each day: generated by tools/board.mjs; do not edit.", as_of: today, drops }, null, 1) + "\n");
