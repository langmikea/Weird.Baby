...
if (CAT.run?.start) {
  ...
  for (const [n, rows] of Object.entries(byDay)) {
    ...
    if (Number(n) === 1) drop(date, "reel", "Ask reel, noon");
    if (who) drop(date, "reel", `Shoot reel: ${who.name}`);
  }
  const last = addDays(CAT.run.start, Math.max(...Object.keys(byDay).map(Number)) - 1);
  for (let d = addDays(CAT.run.start, 1); d <= last; d = addDays(d, 1)) drop(d, "qa", "Q&A, 5:00");
}
