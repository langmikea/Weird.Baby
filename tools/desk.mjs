...
const week = (w, label) => `<li><b>${label || `Week ${w}`}</b><span>Determination: ${esc(count(laneRows("determinations", w)))}<br>Number: ${esc(count(laneRows("numbers", w)))}</span></li>`;
...
<ul class="line">${week(wk)}${week(launchWk, `Launch run, week ${launchWk}`)}${costLine}</ul>
<h2>What you use</h2>
